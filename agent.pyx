...
class Agent:
    def play(self, initial_board : Board, initial_turn, depth=50, count=100, parallel=None, timeout=None, previous_move=None, previous_score=0):
...
class GeneticAgent(Agent):
    ...
    def monte_carlo_tree_search(self, initial_board : Board, initial_turn, previous_move=None, previous_score=0, depth=50, count=100):
        moves, move_probability = self.get_moves_and_probs(initial_board, initial_turn, previous_move, previous_score)
    ...
    def run_one_instance(self, pool_info):
        initial_board, initial_turn, max_depth, timeout, previous_move, previous_score = pool_info
        start_time = time()
        moves, move_probability = self.get_moves_and_probs(initial_board, initial_turn, previous_move, previous_score)
        current_scores = np.zeros((len(moves),))
        while True:
    ...
    def parallel_monte_carlo_tree_search(self, initial_board, initial_turn, parallel, timeout, max_depth=50, previous_move=None, previous_score=0):
        moves = initial_board.get_legal_moves(initial_turn)
        final_scores = np.zeros((len(moves),))
        pool_infos = [(initial_board, initial_turn, max_depth, timeout, previous_move, previous_score)] * parallel
    ...
    def play(self, initial_board : Board, initial_turn, depth=50, count=100, parallel=None, timeout=None, previous_move=None, previous_score=0):
        if parallel is None:
            ...
            return self.parallel_monte_carlo_tree_search(initial_board, initial_turn, parallel, timeout, max_depth=depth, previous_move=previous_move, previous_score=previous_score)
